...
def createProperty():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            newProperty = Property(title,bedNum,bathNum,location,price,type,description,filename)
            db.session.add(newProperty)
            db.session.commit()

            flash("Successfully Added New Property", 'success')
            return redirect(url_for('viewProperties'))
    else:
        flash_errors(form)

    return render_template('addProperties.html', form=form)
